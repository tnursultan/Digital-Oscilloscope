// Load_EFM8LB1.c: This program loads the bootloader into a EFM8LB1 using an AT89LP51RC2
//
...
#define  PAGE_SIZE         0x200L
#define  NUM_PAGES         FLASH_SIZE/PAGE_SIZE // Number of 512-byte FLASH pages
...
#define  NUM_BLOCKS        ((FLASH_SIZE-(2*PAGE_SIZE))/BLOCK_SIZE)
...
#define FGO   P4_5 // Go push button
#define PWR   P2_5 // Power on/off for target controller 0:on
#define GLED  P3_6
#define YLED  P3_7
#define RLED  P2_4
...
#define  C2D_DriverOn      P2M1|=0x02; // Configures C2D pin as push-pull output
#define  C2D_DriverOff     P2M1 &= ~(0x02); P2 |= 0x02  // Configures C2D pin as input with weak pull-up
...
// This bootloader was obtained from a working EFM8LB1.
...
void C2_Init(void)
{
   ...
   Timer0us(2);   // Delay for at least 2us
...
void main (void)
{
	unsigned int i, j, k; // Misc. counters

	printf("\x1b[2JThis program loads the bootloader into a EFM8LB1 using an AT89LP51RC2.\n"
	...
	while(1)
	{
		...
		for (i=NUM_BLOCKS-(PAGE_SIZE/BLOCK_SIZE); i<NUM_BLOCKS; i++) // Perform block reads (0x0000 to 0xFBFF)
		{
		...
		printf("Copying bootloader to target");
		for (i=NUM_BLOCKS-(PAGE_SIZE/BLOCK_SIZE), k=0; i<NUM_BLOCKS; i++, k++) // Perform block write/reads
		{
			// Write FLASH block
			FLASH_ADDR = i*BLOCK_SIZE;
			NUM_BYTES = BLOCK_SIZE;
			memcpy(W_BUF, &Bootloader[k*BLOCK_SIZE], BLOCK_SIZE);
